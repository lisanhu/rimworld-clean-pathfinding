...
namespace CleanPathfinding
{
	...
	public static class Patch_GetBuildingCost
	{
		...
		{
			return (
				(pawn?.Faction?.def.isPlayer ?? false) && //Is pawn the player's?
	...
	public class MapComponent_DoorPathing : MapComponent
	{
		...
		public void RegisterAvoidArea(Area area, bool quiet = false)
		{
			...
			// }
			for (int i = 0; i < Find.CurrentMap.AllCells.Count(); i++)
			{
				if (avoidArea[i] && doorCostGrid[i] == 0)
				{
					doorCostGrid[i] = 45;
				}
			}
		...
		public int ValidateRoomDoors(IntVec3 c, bool roomUpdate = false)
		{
			try
			{
				...
				if (roomUpdate)
				{
					...
					if (tmp?.door != null) c = tmp.door.Position;
					else return 0;
				}
					...
					if (room != null && !room.TouchesMapEdge)
					{
						...
						foreach (var adjacentRegion in room.Regions)
						{
							...
							foreach (RegionLink roomLink in adjacentRegion.links)
							{
								...
								{
									if (logging && Prefs.DevMode) map.debugDrawer.FlashCell(otherDoorRegion.door.Position, text: "\n\nDOOR");
