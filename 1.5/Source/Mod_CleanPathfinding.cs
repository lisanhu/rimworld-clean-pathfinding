...
namespace CleanPathfinding
{
	...
	public static class Setup
	{
		...
		static void SafetyCheck()
		{
            ...
            for (int i = list.Count; i-- > 0;)
            {
                string name = list[i].PackageIdPlayerFacing;
		...
		public override void WriteSettings()
		{
			...
			try
			{
				...
				if (!wanderTuning && patchLedger[nameof(Patch_JobGiver_Wander)])
				{
					...
					harmony.Unpatch(AccessTools.Method(typeof(JobGiver_Wander), "TryGiveJob" ), HarmonyPatchType.Postfix, this.Content.PackageIdPlayerFacing);
				}
				else if (wanderTuning && !patchLedger[nameof(Patch_JobGiver_Wander)])
				{
					patchLedger[nameof(Patch_JobGiver_Wander)] = true;
					harmony.Patch(AccessTools.Method(typeof(JobGiver_Wander), "TryGiveJob" ), 
