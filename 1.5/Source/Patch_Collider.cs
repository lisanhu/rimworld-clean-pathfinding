...
namespace CleanPathfinding
{
    ...
    static class Patch_ShouldCollideWithPawns
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            yield return new CodeInstruction(OpCodes.Ldfld, AccessTools.Field(typeof(Pawn), "mindState"));
            yield return new CodeInstruction(OpCodes.Ldfld, AccessTools.Field(typeof(Pawn_MindState), "anyCloseHostilesRecently"));
            yield return new CodeInstruction(OpCodes.Brfalse_S, label);
            
            yield return new CodeInstruction(OpCodes.Ldarg_0);
            yield return new CodeInstruction(OpCodes.Ldfld, AccessTools.Field(typeof(Pawn), nameof(Pawn.health)));
            yield return new CodeInstruction(OpCodes.Ldfld, AccessTools.Field(typeof(Pawn_HealthTracker), "healthState"));
