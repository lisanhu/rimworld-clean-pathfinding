	...
	public static class CleanPathfindingUtility
	{
		...
		static public float AdjustCosts(Pawn pawn, TerrainDef def, float cost, Map map, int index)
		{
			...
			if (pawn.thingIDNumber != lastPawnID)
			{
				...
				(factorBleeding && pawn.health.hediffSet.BleedRateTotal > 0.1f)); //They are bleeding
			...
			else
			{
				...
				if (terrainCache.TryGetValue(def.shortHash, out lastTerrainCacheCost))
				{
					...
					if (lastTerrainCacheCost > 0 && pawn.Faction != null && pawn.Faction.def.isPlayer) lastTerrainCacheCost = 0;
			...
			{
				++calls;
				if (Current.Game.tickManager.TicksGame != loggedOnTick)
				{
					loggedOnTick = Current.Game.tickManager.TicksGame;
				...
				// it's a DebugCell is a sealed class and I don't know how to do it now
				map.debugDrawer.FlashCell(cell, (float)cost, cost.ToString(), 50);
			}
		skipAdjustment:
			if (cost < 0) return 0;
			return cost;
			...
			{
				//Check and set cache
				if (Current.Game.tickManager.TicksGame % 600 == 0) lastFactionID = -1; // Reset every 10th second
				...
				FieldInfo _relations = typeof(Faction).GetField("relations", BindingFlags.Instance | BindingFlags.NonPublic);
				List<FactionRelation> relations = _relations.GetValue(faction) as List<FactionRelation>;
				for (int i = relations.Count; i-- > 0;)
				{
					var tmp = relations[i];
					if (tmp.other == Current.Game.World.factionManager.OfPlayer)
					{
			...
			float GameGlowAtFast(Map map, int index)
			{
				...
				if (map.roofGrid.RoofAt(index) == null)
				{
					daylight = map.skyManager.CurSkyGlow;
					if (daylight == 1f) return 1f;
				}

				// ColorInt color = map.glowGrid.glowGrid[index];
				FieldInfo _cachedAccumulatedGlow = typeof(GlowGrid).GetField("cachedAccumulatedGlow", BindingFlags.Instance | BindingFlags.NonPublic);
				Color32?[] cachedAccumulatedGlow = _cachedAccumulatedGlow.GetValue(map.glowGrid) as Color32?[];
				ColorInt color = new ColorInt(cachedAccumulatedGlow[index].Value);
